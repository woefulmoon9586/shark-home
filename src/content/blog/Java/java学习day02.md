...
```java
public static double calculateBMI(double height, double weight) { return weight / (height * height); }
```
## 基础语法-方法的注意事项
...
```java
import java.util.Scanner; public class InputExample { public static void main(String[] args) { Scanner scanner = new Scanner(System.in); System.out.print("请输入一个数字："); int number = scanner.nextInt(); System.out.println("你输入的数字是：" + number); scanner.close(); } }
```
## 运算符-基本运算符-自增自减运算符
...
```java
int count = 0; count++; // count 变为 1 count--; // count 变为 0
```
## 运算符-赋值运算符-三元运算符
...
```java
boolean isAdult = age >= 18; boolean isEmployed = isAdult && hasJob;
```
## 综合实战-身体健康指数计算
